...
public:
    buffer_object()
            : bindable([](auto& i) { juce::gl::glGenBuffers(1, &i); },
                       [](auto i) { juce::gl::glDeleteBuffers(1, &i); }) {
    }
    ...
    void sub_data(size_t item_offset, const T* ptr, size_t items) {
        ...
        const auto s = this->get_scoped();
        juce::gl::glBufferSubData(type, byte_offset, in_buffer_bytes, ptr);
    }
    ...
    void data(const T* ptr, size_t items) {
        if (items == num_elements) {
            ...
            const auto s = this->get_scoped();
            juce::gl::glBufferData(type, size_in_bytes(), ptr, mode);
        }
    }
...
private:
    void do_bind(GLuint index) const override {
        juce::gl::glBindBuffer(type, index);
    }
...
using static_vbo = buffer_object<juce::gl::GL_ARRAY_BUFFER, juce::gl::GL_STATIC_DRAW>;
using static_ibo = buffer_object<juce::gl::GL_ELEMENT_ARRAY_BUFFER, juce::gl::GL_STATIC_DRAW>;
using dynamic_vbo = buffer_object<juce::gl::GL_ARRAY_BUFFER, juce::gl::GL_DYNAMIC_DRAW>;
using dynamic_ibo = buffer_object<juce::gl::GL_ELEMENT_ARRAY_BUFFER, juce::gl::GL_DYNAMIC_DRAW>;
